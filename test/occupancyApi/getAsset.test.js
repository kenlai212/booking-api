...
describe('Test get occupancy', () => {
    it("missing occupacyId, 404 error!", async () => {
        try{
            await axios.get(`${occupancyTestCommon.OCCUPANCY_DOMAIN_URL}/occupancy`, REQUEST_CONFIG);
        }catch(error){
            expect.assertions(1);
    ...
    it("invalid occupacyId, 404 error!", async () => {
        ...
        try{
            await axios.get(`${occupancyTestCommon.OCCUPANCY_DOMAIN_URL}/occupancy/NON_EXIST`, REQUEST_CONFIG);
        }catch(error){
            expect.assertions(1);
            expect(error.response.status).toEqual(404);
        }
    ...
    it("valid occupacyId, 200!", async () => {
        ...
        const postOccupancy2Request = {
            startTime: occupancyTestCommon.getTomorrowTenAMDate(),
            endTime: occupancyTestCommon.getTomorrowElevenAMDate(),
            utcOffset:0,
            assetType:"BOAT",
            assetId:"A456",
            referenceType:"MAINTAINANCE"
        }
        const postOccupancy2Response = await axios.post(`${occupancyTestCommon.OCCUPANCY_DOMAIN_URL}/occupancy`, postOccupancy2Request, REQUEST_CONFIG);
        const getOccupancy2Response = await axios.get(`${occupancyTestCommon.OCCUPANCY_DOMAIN_URL}/occupancy/${postOccupancy2Response.data.occupancyId}`, REQUEST_CONFIG);
